...
class ScrollableGui
{
    ...
    static register(hWnd_or_guiObj, innerScrollOnFocus:=true)    {
        ...
        ,this._opt[hWnd] := {innerScrollOnFocus:(!!innerScrollOnFocus),wheelResidual:map()}
        ;-----------------------------------
        ,lpsi:=buffer(28,0)
    ...
    static updateBoundary(hWnd_or_guiObj, newWidth?, newHeight?, setMaxSize:=true)    {
        ...
        if (!this._registerBoundarySize(hWnd,,, (isSet(newWidth) ? border.left + newWidth : unset), (isSet(newHeight) ? border.top + newHeight : unset)))
            return false
        if (setMaxSize)    {
            showOptions:=""
    ...
    */
    ...
    ;--------------------------------------------------
    static _onScroll(hContainerWnd, wParam, lParam, Msg, hWnd)    {
    ...
    static _consumeWheelDelta(hContainerWnd, hTargetWnd, scrollMsg, wheelMsg, wheelDistance)    {
        static WHEEL_DELTA:=120
        residuals:=this._opt[hContainerWnd].wheelResidual
        ,key:=(hTargetWnd&0xffffffff) "|" scrollMsg "|" wheelMsg
        ,residual:=(residuals.has(key)?residuals[key]:0)+wheelDistance
